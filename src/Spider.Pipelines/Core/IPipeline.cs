namespace Spider.Pipelines.Core
{
    ...
    public interface IPipeline<TRequest>
    {
        ...
        /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
        /// <returns>A task representing the asynchronous operation.</returns>
        Task RunAsync(TRequest request, Action<IExecutionBoundaryCollection<TRequest>> configureExecution, CancellationToken cancellationToken = default);
    }
    ...
    public interface IPipeline<TRequest, TResponse>
    {
        ...
        /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
        /// <returns>A task representing the asynchronous operation, with the response as its result.</returns>
        Task<TResponse> RunAsync(TRequest request, Action<IExecutionBoundaryCollection<TRequest, TResponse>> configureExecution, CancellationToken cancellationToken = default);
    }
}
